...
$data['clientes'] = $conexion->query("SELECT COUNT(DISTINCT cliente_id) as total FROM ventas")->fetch_assoc()['total'] ?? 0;
// Cambiamos SUM(cantidad) por COUNT(*) para que cuente los registros
$data['productos_vendidos'] = $conexion->query("SELECT COUNT(*) as total FROM detalle_ventas")->fetch_assoc()['total'] ?? 0;
